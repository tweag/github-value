...
import { DailyMetric, Editor, IdeCompletions, LanguageStats, ModelStats } from '../models/metrics.model.new';
import { Op } from 'sequelize';

class MetricsControllerNew {
  // Get all metrics 📊
  async getAllMetrics(req: Request, res: Response): Promise<void> {
    const { since, until, editor, language, model } = req.query;
    try {
      ...
      console.log(Object.getOwnPropertySymbols(dateFilter).length, { date: dateFilter });
